...
export class PointerController {
  ...
  pointerDown(e: PointerEvent) {
    ...
    if (this.pointers.size === 2) { this.resetSinglePointerState(); this.wasPinching = true; this.startPinch(); return; }
    if (this.pointers.size > 2)  return;
  ...
  pointerUp(e: PointerEvent) {
    ...
    if (this.pointers.size <= 1) {
      const wasTap = !this.isPanning && !this.isDragging && !this.isDeleting && !this.wasPinching;
      if (wasTap && this.pointers.size === 0) {
        const [cx, cy] = this.cellAt(e.clientX, e.clientY);
        if (e.pointerType === 'mouse' && e.button === 2) this.cb.onDelete(cx, cy);
        else                                              this.cb.onPlace(cx, cy, false);
      }
      if (this.pointers.size === 0) { this.wasPinching = false; this.resetSinglePointerState(); }
    }
  }
