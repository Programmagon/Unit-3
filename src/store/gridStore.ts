...
interface GridStore {
  grid: Grid; stepCount: number; isRunning: boolean; hz: number; loopError: string | null;
...
export const useGridStore = create<GridStore>((set, get) => ({
  grid: new Map(), stepCount: 0, isRunning: false, hz: 5, loopError: null,

  setCell: (x, y, type, state = false) => set(s => {
    const g = new Map(s.grid); g.set(key(x, y), { type, state }); return { grid: g };
  }),
